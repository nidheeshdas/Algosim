...
class Listener: public PTYPES_NAMESPACE::thread
{
public:
	/// By default, listener not registered
	Listener() :
		_registered(false), _ignoringHeardEvent(false), _processingEvent(false), thread(false)
	{
	}
	PTYPES_NAMESPACE::jobqueue jobs;
...
template<typename EvType>
inline void Listener<EvType>::preProcessEvent()
{
...
template<typename EvType>
inline void Listener<EvType>::postProcessEvent()
{
 ...
 */
template<typename EvType>
inline void Listener<EvType>::processEventPublic(const EvType& event)
{
	preProcessEvent();
	try
	{
