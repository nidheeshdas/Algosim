...
public:
	...
	{
		listenForEvents();
		string imgPath = cur_sim->basePath + "/1two_compl.jpg";
	...
	void nextStep()
	{
		...
		ugc.printAtXY(itobs(s.values[3], 1), 60, 70);
	}
	void process(const state& s)
	{
		...
		cm.Show(s);
		gc << "--------------------------------------------------------------------------------------\n";
		gc << "|No |          Step         |  Accumulator     |    RegisterQ     |    RegisterM     |\n";
		gc << "--------------------------------------------------------------------------------------\n";
		int p = current_state;
		if (p != 0)
		{
			...
			gc << "| " << setw(2) << p + 1 << "|" << setw(23) << left << it.StepName << "| " << setw(16) << left << itobs(it.values[0], 16) << " | "
					<< setw(16) << left << itobs(it.values[1], 16) << " | " << setw(16) << left << itobs(it.values[2], 16) << " |\n";
			if (p != (memory.size() - 1))
				p++;
		}
		state it = memory[p];
